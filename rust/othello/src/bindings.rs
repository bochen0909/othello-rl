...
use othello_engines::{
    compute_move_aelskens, compute_move_drohh, compute_move_nealetham,
    compute_move_scores_aelskens, compute_move_scores_drohh, compute_move_scores_nealetham,
};
...
/// Compute move scores for aelskels engine
///
/// Args:
///     board (list): Flat board state as 64 elements (0=Empty, 1=Black, 2=White)
///     player (int): Current player (1=Black, 2=White)
///
/// Returns:
///     list: Array of 64 scores (int) where legal moves have non-zero scores
#[pyfunction]
fn compute_move_scores_aelskels_py(board: Vec<u8>, player: u8) -> PyResult<Vec<i32>> {
    if board.len() != 64 {
        return Err(PyValueError::new_err("board must have 64 elements"));
    }

    let mut board_array: [u8; 64] = [0; 64];
    board_array.copy_from_slice(&board);

    Ok(compute_move_scores_aelskens(&board_array, player).to_vec())
}

/// Compute move scores for drohh engine
///
/// Args:
///     board (list): Flat board state as 64 elements (0=Empty, 1=Black, 2=White)
///     player (int): Current player (1=Black, 2=White)
...
///     list: Array of 64 scores (int) where legal moves have non-zero scores
#[pyfunction]
fn compute_move_scores_drohh_py(board: Vec<u8>, player: u8) -> PyResult<Vec<i32>> {
    if board.len() != 64 {
        return Err(PyValueError::new_err("board must have 64 elements"));
    }
    ...
    Ok(compute_move_scores_drohh(&board_array, player).to_vec())
}

/// Compute move scores for nealetham engine
///
/// Args:
///     board (list): Flat board state as 64 elements (0=Empty, 1=Black, 2=White)
///     player (int): Current player (1=Black, 2=White)
...
///     list: Array of 64 scores (int) where legal moves have non-zero scores
#[pyfunction]
fn compute_move_scores_nealetham_py(board: Vec<u8>, player: u8) -> PyResult<Vec<i32>> {
    if board.len() != 64 {
        return Err(PyValueError::new_err("board must have 64 elements"));
    }
    ...
    Ok(compute_move_scores_nealetham(&board_array, player).to_vec())
}
...
fn othello_rust(_py: Python, m: &PyModule) -> PyResult<()> {
    ...
    m.add_function(wrap_pyfunction!(compute_move_scores_aelskels_py, m)?)?;
    m.add_function(wrap_pyfunction!(compute_move_scores_drohh_py, m)?)?;
    m.add_function(wrap_pyfunction!(compute_move_scores_nealetham_py, m)?)?;
    Ok(())
}
