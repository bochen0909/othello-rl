...
class TestObservationEncodingProperties:
    ...
    def test_observation_encoding_correctness(self, moves, agent_player):
        ...
        **Validates: Requirements 4.1, 4.2, 4.3, 4.4, 4.6**
        ...
        overlap = obs[0] * obs[1]
        assert np.sum(overlap) == 0, "Agent and opponent channels overlap"

        # Property 8: Verify all values are binary (0 or 1)
        assert np.all((obs == 0) | (obs == 1)), "Observation contains non-binary values"
    ...
    def test_observation_valid_moves_are_actually_valid(self, moves):
        ...
        assert np.array_equal(
            valid_moves_from_obs, valid_moves_from_game.astype(np.float32)
...
class TestInfoDictionaryProperties:
    ...
    def _verify_info_dict(self, env, info):
        ...
        assert info["black_count"] >= 0, f"Negative black_count: {info['black_count']}"
        assert info["white_count"] >= 0, f"Negative white_count: {info['white_count']}"

        # Property 5: piece counts sum to at most 64
        total_pieces = info["black_count"] + info["white_count"]
        assert total_pieces <= 64, f"Total pieces {total_pieces} exceeds 64"
...
class TestStepMethodProperties:
    ...
    def test_step_return_signature(self, action):
        ...
        **Validates: Requirements 3.3**
        ...
        assert truncated is False, "truncated should always be False for Othello"
    ...
    def test_invalid_move_handling(self, moves, invalid_mode):
        ...
        **Validates: Requirements 3.7, 5.3**
        ...
        if invalid_mode == "error":
            ...
            assert not terminated, "Game terminated after invalid move in penalty mode"
            ...
            assert (
                black_count_before != black_count_after
                or white_count_before != white_count_after
                or terminated
        ...
        **Validates: Requirements 5.1**
        ...
        for move in moves:
            ...
            if not terminated:
                ...
                if winner == 2:  # Draw
                    assert reward == 0.0, f"Draw reward should be 0, got {reward}"
                elif winner == env.agent_player:  # Agent wins
                    assert reward == 1.0, f"Win reward should be 1.0, got {reward}"
                else:  # Agent loses
                    assert reward == -1.0, f"Loss reward should be -1.0, got {reward}"
    ...
    def test_dense_reward_correctness(self, moves, agent_player):
        ...
        **Validates: Requirements 5.2, 5.5**
        ...
        for move in moves:
            ...
            # Use agent_player_moved which is the player that made this move
            if info["agent_player_moved"] == 0:  # Agent was Black for this move
                expected_reward = (black_count - white_count) / 64.0
            else:  # Agent was White for this move
            ...
            assert -1.0 <= reward <= 1.0, f"Dense reward {reward} out of range [-1, 1]"
    ...
    def test_custom_reward_function(self, moves):
        ...
        **Validates: Requirements 5.4**
        ...
        for move in moves:
            ...
            black_count, white_count = env.game.get_piece_counts()
            # Use agent_player_moved which is the player that made this move
            if info["agent_player_moved"] == 0:  # Agent was Black
                expected_reward = (black_count - white_count) * 0.01
            else:  # Agent was White
                expected_reward = (white_count - black_count) * 0.01
    ...
    def test_custom_reward_function_receives_correct_state(self, action):
        ...
        if valid_moves[action]:
            agent_player_before = env.agent_player  # Track agent_player before step
            obs, reward, terminated, truncated, info = env.step(action)

            # Property 1: Custom function was called
            assert len(received_states) > 0, "Custom reward function was not called"
            ...
            for field in required_fields:
                assert field in state, f"Missing required field in game state: {field}"

            # Property 3: State values match game state
            black_count, white_count = env.game.get_piece_counts()
            assert state["black_count"] == black_count
            assert state["white_count"] == white_count
            # The reward function receives agent_player from BEFORE the step (stored in info["agent_player_moved"])
            assert state["agent_player"] == info["agent_player_moved"]
...
class TestSelfPlayProperties:
    ...
    def test_callable_opponent_policy(self, moves):
        ...
        if np.any(valid_moves_before):
            ...
            assert call_count[0] > 0, "Callable opponent was not called"
    ...
    def test_greedy_opponent_maximizes_flips(self, opponent_type):
        ...
        piece_diff = abs(
            (black_count_after - black_count_before)
        ...
        assert piece_diff > 0, "Greedy opponent didn't flip any pieces"
...
class TestRenderingProperties:
    ...
    def test_ansi_rendering_completeness(self, moves):
        ...
        **Validates: Requirements 10.2, 10.4, 10.5**
        ...
        assert len(ansi_output) > 0, "ANSI rendering returned empty string"
        ...
        assert "0 1 2 3 4 5 6 7" in ansi_output, "ANSI rendering missing column headers"

        # Property 7: Contains row numbers (0-7)
        for i in range(8):
            assert f"{i} " in ansi_output, f"ANSI rendering missing row number {i}"

        # Property 8: Valid moves are marked with *
        valid_moves = env.game.get_valid_moves().reshape(8, 8)
        if np.any(valid_moves):
            # If there are valid moves, should contain * marker
            assert "*" in ansi_output, "ANSI rendering missing valid move markers (*)"
    ...
    def test_rgb_array_format(self, moves):
        ...
        **Validates: Requirements 10.3**
        ...
        assert np.all(rgb_output >= 0), "RGB array contains values < 0"
        assert np.all(rgb_output <= 255), "RGB array contains values > 255"

        # Property 6: Array is not all zeros (should have some content)
        assert np.any(rgb_output > 0), "RGB array is all zeros"
...
class TestStatePersistenceProperties:
    ...
    def test_state_persistence_round_trip(self, moves):
        ...
        **Validates: Requirements 11.7**
        ...
        assert np.array_equal(obs1, obs2), "Observations don't match after save/load"
    ...
    def test_save_load_preserves_agent_player(self, moves, agent_player):
        ...
        # Save the current agent_player AFTER moves (not the initial one)
        expected_agent_player = env.agent_player
        ...
        assert env2.agent_player == expected_agent_player, (
            f"agent_player not preserved: {env2.agent_player} != {expected_agent_player}"
        )
    ...
    def test_save_load_initial_state(self):
        ...
        assert state["black_count"] == 2, "Initial state should have 2 black pieces"
        assert state["white_count"] == 2, "Initial state should have 2 white pieces"
    ...
    def test_load_state_allows_continued_play(self, moves):
        ...
        if np.any(valid_moves):
            ...
            assert obs.shape == (3, 8, 8), "Observation has wrong shape after load"
            assert isinstance(reward, (float, np.floating)), (
...
class TestEpisodeTerminationProperties:
    ...
    def test_reset_clears_termination(self):
        ...
        if np.any(valid_moves):
            ...
            assert terminated is False, "Game terminated immediately after reset"
            assert truncated is False, "Game truncated immediately after reset"
